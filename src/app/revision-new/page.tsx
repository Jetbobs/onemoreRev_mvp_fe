...
function RevisionPageContent() {
  ...
  const submitRevision = async () => {
    ...
    try {
      ...
      const result = await response.json()
      console.log('[revision-new] submit success:', result)
      ...
      console.error('[revision-new] submit error:', error)
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-6">
        ...
        {!code && (revision?.invitationCode || revision?.project?.invitationCode) && (
          <div className="mb-4">
            <Link
              href={`/revision-new?projectId=${projectId}&revNo=${revNo}&code=${revision.invitationCode || revision.project?.invitationCode}`}
              target="_blank"
